    ...
    return <div className={style.issue + ' ' + (focused ? style.focused : '')}>
        <div className={style.container} onClick={handleEdit}>
            ...
            <div className={style.content}>
                ...
                <p className={style.author}><span className={style.avatar} style={{ backgroundImage: `url(${author.avatar})` }} />{formatDate(creationDate)} by {author?.name}</p>
        ...
        {isEdited && <IssueEditModal action={(props.author.name === user.name && !props.resolved) ? 'edit' : 'view'} refresh={handleRefresh} handleComplete={handleClose} handleReopen={handleOpen} handleDelete={handleDelete} issue={props} isOpen={isEdited} onRequestClose={handleEditCompleted} />}
